...
class RecommenderSystem:
    ...
    def clean_data(self, min_item_interactions=3):
        """
        Clean the dataset by removing cold items (< min_item_interactions)
        and users who end up with no remaining interactions.
        """
        print("\n" + "="*60)
        print(f"DATA CLEANING (threshold={min_item_interactions})")
        print("="*60)

        # Count item occurrences over the current raw user_items
        item_counts = Counter()
        for items in self.user_items.values():
            item_counts.update(items)

        # Decide which items to keep
        keep_items = {it for it, c in item_counts.items() if c >= min_item_interactions}
        dropped_items = len(item_counts) - len(keep_items)
        print(f"  Dropping {dropped_items} cold items out of {len(item_counts)} total.")

        # Filter user interactions; drop users with no remaining items
        cleaned_user_items = {}
        for user, items in self.user_items.items():
            filtered = {it for it in items if it in keep_items}
            if filtered:
                cleaned_user_items[user] = filtered

        dropped_users = len(self.user_items) - len(cleaned_user_items)
        self.user_items = cleaned_user_items

        # Rebuild item_users from cleaned data
        self.item_users.clear()
        for user, items in self.user_items.items():
            for it in items:
                self.item_users[it].add(user)

        print(f"  Remaining users: {len(self.user_items)}")
        print(f"  Remaining items: {len(self.item_users)}")
        print(f"  Users dropped: {dropped_users}")
        print(f"  Items dropped: {dropped_items}")
        print("="*60 + "\n")
    ...
    def build_user_item_matrix(self, normalize=True):
        """
        ...
        for user_id, items in self.user_items.items():
            ...
            for item_id in items:
                if item_id in self.item_to_idx:
                    ...
                    if normalize:
                        # TF-IDF-like normalization: less weight for power users
    ...
    def recommend_items_item_based(self, user_id, n_recommendations=20, popularity_penalty=-1, use_svd=False, use_als=False, use_ensemble=False):
        """
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--evaluate' or True:
        # EVALUATION MODE
        print("\n" + "="*60)
        ...
        recommender.clean_data(min_item_interactions=0)
        ...
        recommender.load_raw_data("train-2.txt")
        recommender.perform_eda()
        recommender.clean_data(min_item_interactions=3)
        recommender.build_user_item_matrix()
